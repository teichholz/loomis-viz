...
function Vis() {
  ...
  const [selected, setSelected] = useState<Point2>();
  ...
  function resize() {
    const depth = 0.8;
    const diameter = Math.min(visibleHeightAtZDepth(depth, camera as THREE.PerspectiveCamera), visibleWidthAtZDepth(depth, camera as THREE.PerspectiveCamera));
  ...
  return (
    <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-geist-sans)]">
      <main className="flex flex-col gap-8 row-start-2 items-center sm:items-start h-4/5 w-4/5">
        <Canvas camera={{fov: 65}} className="" style={{ "cursor": cursor }}>
          <Vis />
        </Canvas>
      </main>
    </div>
